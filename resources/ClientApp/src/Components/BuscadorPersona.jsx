...
const defaultPersona = {id: 0, nombres: "Sin datos", apellidos: "", imagenes: [], lugar_nacimiento: {} };
const BuscadorPersona = ({ selectMultiple = false, data = [defaultPersona], imgDefault, imgOrigin = "", selectableProperty = "id", selected = defaultPersona, arraySelected = [defaultPersona], handler, pageSize = 8, pageExtends = 1, totalRows = 0, search = "", handlerSelectPersona, handlerChangeSearch}) => {
            ...
            <div className="BuscadorPersonaContainer">
                {
                    ...
                    data.map((persona, i) => {
                        ...
                        return (
                            ...
                            }} key={i} className={`personaItem ${selected.id === persona.id ? "selected" : ""}`}>
                                <div className="photo">
                                    <img src={persona.imagenes[0] !== undefined ? imgOrigin + persona.imagenes[0].imagen : imgDefault || ""} />
                                </div>
                                <div className="name">
                                    <p>{`${persona.nombres || ""} ${persona.apellidos || ""}`}</p>
                                </div>
                                <div className="estate">
                                    <p>{`${persona.lugar_nacimiento?.nombre || ""}`}</p>
                                </div>
                            </div>
                        );
                    })
