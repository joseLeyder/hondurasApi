...
let classTh = "";
const TableReactExtends = ({ columns, data, hiddenColumns, handler, pageExtends = 1, pageSizeExtends = 5, totalRows = 0, search = "", handlerForEachRow = "", handlerForEachCell = "",TypeSearch="text" }) => {
                   ...
                   
                    {
                        ...
                        <DatePicker
                            ...
                            onChangeDate={async (e) => {
                                pageExtends = 1;
                                await handler(pageExtends, pageSize, e, true)}}
                                spanClass="error"
                                spanError={''}
                                divClassSpanType={1}
                                divClassSpan="input-group-addon"
                                divClassSpanI="fa fa-search"/>
                        :
                ...
                <table className="TableReactResponsive table table-responsive table-striped" {...getTableProps()}>
                    ...
                    <tbody {...getTableBodyProps()}>
                        {page.map((row, i) => {
                            ...
                            return (
                                <tr {...row.getRowProps()}>
                                    <td onClick={(e) => { AppendTrChild(e.currentTarget.parentNode, row, document.querySelector(".TableContainer")) }} className="ExpandButton none"><i className="fa fa-caret-right"></i></td>
                                    {row.cells.map(cell => {
...
function AppendTrChild(row, originalRow, tableContainer) {
    document.querySelectorAll(".trChild").forEach(item => {
        item.remove();
    });
    let table = tableContainer.querySelector(".TableReactResponsive");
    ...
    row.querySelectorAll("td.trRemove").forEach((item, i) => {
        let numChild = Array.prototype.indexOf.call(row.children, item);
        let span = document.createElement("span");
        span.innerHTML = item.innerHTML;
        ul.querySelectorAll("li")[i].appendChild(span);

        if(ul.querySelectorAll("li")[i].querySelector("button")){
            let button = ul.querySelectorAll("li")[i].querySelector("button");
            if(originalRow.cells[numChild - 1].column.clickHandler !== null || originalRow.cells[numChild - 1].column.clickHandler !== "" || typeof originalRow.cells[numChild - 1].column.clickHandler !== 'undefined'){
                button.addEventListener("click", originalRow.cells[numChild - 1].column.clickHandler(originalRow.values))
            }
        }
    });
