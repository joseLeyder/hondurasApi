...
const NombreDelModulo = "agendaLegislativaActividades";

class AgendaActividadDataService {
    async create(data) {
    ...
    async getAllProyectosLeySearch(search = "",page = 1,rows = 4) {
        return await http.get(
            `/utils/getAllProyectosLeySearch?search=${search}&page=${page}&rows=${rows}`
        );
    }
    async getAllProyectosLeySearchtotalrecords(search = "") {
        return await http.get(
            `/utils/getAllProyectosLeySearchtotalrecords?search=${search}`
        );
    }  
}

export default new AgendaActividadDataService();
