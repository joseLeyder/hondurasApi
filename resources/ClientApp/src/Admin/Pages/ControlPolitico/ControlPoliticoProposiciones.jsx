...
class ControlPoliticoProposiciones extends Component {
    constructor(props) {
        super(props);
            ...
            tableInfo: {
                columns: [
                    ...
                    },
                    {
                        Header: "Acciones",
                        columns: [
                            auth.tieneModuloPermiso(ModuloPermiso.ControlPolitico.ObtenerProposiciones) ?
                                {
                                    Header: 'Editar',
                                    id: 'editar',
                                    accessor: (str) => 'editar',
                                    Cell: (tableProps) => (
                                        <ValidarPermiso
                                            IdModuloPermisoValidar={ModuloPermiso.ControlPolitico.ObtenerProposiciones}
                                            DefaultTemplate={
                                                <button
                                                    data-toggle="modal"
                                                    data-target="#add-proposicion"
                            ...
                            auth.tieneModuloPermiso(ModuloPermiso.ControlPolitico.EliminarProposiciones) ?
                                {
                                    ...
                                    Cell: (tableProps) => (
                                        <ValidarPermiso
                                            IdModuloPermisoValidar={ModuloPermiso.ControlPolitico.EliminarProposiciones}
                                            DefaultTemplate={
                                                <button
                                                    data-toggle="modal"
    ...
    saveSubmit = async (e) => {
        ...
        if (this.state.fields["id"] === 0) {
            await ControlPoliticoDataService.createProposicion(data)
                .then(response => {
            ...
            await ControlPoliticoDataService.updateProposicion(this.state.fields["id"], data)
                .then(response => {
                    responseData = response.data;
                })
                .catch(function (error) {
    ...
    deleteSubmit = async (e) => {
        ...
        let responseData;
        await ControlPoliticoDataService.deleteProposicion(this.state.fields["id"])
            .then(response => {
                responseData = response.data;
            })
            .catch(function (error) {
    ...
    getAll = async (idFilterActive, page, rows, search) => {
        ...
        let tableInfo = this.state.tableInfo;
        await ControlPoliticoDataService.getAllProposicion(
        ...
        await ControlPoliticoDataService.getTotalRecordsControlPoliticoProposicion(idFilterActive, this.state.id, search)
            .then(response => {
    ...
    getByID = async (id) => {
        this.setState({ loading: true });
        await ControlPoliticoDataService.getProposicion(id)
            .then((response) => {
    ...
    render() {
        return (
            <div>
                ...
                <div className="modal" id="add-proposicion" role="dialog" aria-labelledby="largeModalHead" aria-hidden="true">
                    <div className="modal-dialog modal-lg">
                        <div className="modal-content">
                            <div className="modal-header">
                                ...
                                <h4 className="modal-title" id="largeModalHead"><i className="fa fa-ticket"></i> {this.state.fields["id"] === 0 ? "Nuevo" : "Editar"} cuestionario</h4>
                            </div>
                            <div className="modal-body">
                                <form className="form-horizontal custom" onSubmit={this.saveSubmit.bind(this)}>
                                    <Input divClass="col-md-12" inputName="idProposicion" inputType="hidden" inputValue={this.state.fields["id"]} />
                                    <div className="row">
                                    ...
                                    <div className="row ">
                                        <div className="col-md-12 push-up-30">
                                                ...
                                                }} ref="closemodalSave">Cerrar</button>
                                            {
                                                this.state.fields["id"] === 0 ?
                                                    <ValidarPermiso
                                                        IdModuloPermisoValidar={ModuloPermiso.ControlPolitico.AgregarProposiciones}
                                                        DefaultTemplate={
                                                            <button className="btn btn-success pull-right"><i className="fa fa-check"></i> Guardar </button>
                                                        }
                                                    /> :
                                                    <ValidarPermiso
                                                        IdModuloPermisoValidar={ModuloPermiso.ControlPolitico.ModificarProposiciones}
                                                        DefaultTemplate={
                                                            <button className="btn btn-success pull-right"><i className="fa fa-check"></i> Guardar </button>
                                                        }
                                                    />
                    ...
                    <form action="">
                        <div className="mb-container">
                            <div className="mb-middle">
                                <div className="mb-title"><span className={`fa fa-${this.state.fields.activo ? "eraser" : "check"}`}></span> {this.state.fields.activo ? "Desactivar" : "Activar"} cuestionario</div>
                                <div className="mb-content">
                                    <p>{`¿Está seguro que desea ${this.state.fields.activo ? "desactivar" : "activar"} el cuestionario ${this.state.fields.nombre}? Puede volver a ${this.state.fields.activo ? "activarlo" : "desactivarlo"} en cualquier otro momento.`}</p>
                                </div>
                        ...
                        <div className="rowListado">
                            <div className="panel panel-default ">
                                <div className="panel-heading">
                                    <h3 className="panel-title"><span className="fa fa-list"></span> Listado de cuestionarios</h3>
...
export default ControlPoliticoProposiciones;