...
class ControlPoliticoRespuestas extends Component {
    constructor(props) {
        super(props);
            ...
            tableInfo: {
                columns: [
                    ...
                    },
                    {
                        Header: "Acciones",
                        columns: [
                            auth.tieneModuloPermiso(ModuloPermiso.ControlPolitico.ObtenerRespuestas) ?
                                {
                                    Header: 'Editar',
                                    id: 'editar',
                                    accessor: (str) => 'editar',
                                    Cell: (tableProps) => (
                                        <ValidarPermiso
                                            IdModuloPermisoValidar={ModuloPermiso.ControlPolitico.ObtenerRespuestas}
                                            DefaultTemplate={
                                                <button
                                                    data-toggle="modal"
                                                    data-target="#add-respuesta"
                            ...
                            auth.tieneModuloPermiso(ModuloPermiso.ControlPolitico.EliminarRespuestas) ?
                                {
                                    ...
                                    Cell: (tableProps) => (
                                        <ValidarPermiso
                                            IdModuloPermisoValidar={ModuloPermiso.ControlPolitico.EliminarRespuestas}
                                            DefaultTemplate={
                                                <button
                                                    data-toggle="modal"
    ...
    saveSubmit = async (e) => {
        ...
        if (this.state.fields["id"] === 0) {
            await ControlPoliticoDataService.createRespuesta(data)
                .then(response => {
            ...
            await ControlPoliticoDataService.updateRespuesta(this.state.fields["id"], data)
                .then(response => {
                    responseData = response.data;
                })
                .catch(function (error) {
    ...
    deleteSubmit = async (e) => {
        ...
        let responseData;
        await ControlPoliticoDataService.deleteRespuesta(this.state.fields["id"])
            .then(response => {
                responseData = response.data;
            })
            .catch(function (error) {
    ...
    getAll = async (idFilterActive, page, rows, search) => {
        ...
        let tableInfo = this.state.tableInfo;
        await ControlPoliticoDataService.getAllRespuesta(
        ...
        await ControlPoliticoDataService.getTotalRecordsControlPoliticoRespuesta(idFilterActive, this.state.id, search)
            .then(response => {
    ...
    getByID = async (id) => {
        this.setState({ loading: true });
        await ControlPoliticoDataService.getRespuesta(id)
            .then((response) => {
    ...
    render() {
        return (
            <div>
                ...
                <div className="modal" id="add-respuesta" role="dialog" aria-labelledby="largeModalHead" aria-hidden="true">
                    <div className="modal-dialog modal-lg">
                        <div className="modal-content">
                            <div className="modal-header">
                                ...
                                <h4 className="modal-title" id="largeModalHead"><i className="fa fa-ticket"></i> {this.state.fields["id"] === 0 ? "Nueva" : "Editar"} respuesta</h4>
                            </div>
                            <div className="modal-body">
                                <form className="form-horizontal custom" onSubmit={this.saveSubmit.bind(this)}>
                                    <Input divClass="col-md-12" inputName="idRespuesta" inputType="hidden" inputValue={this.state.fields["id"]} />
                                    <div className="row">
                                    ...
                                    <div className="row ">
                                        <div className="col-md-12 push-up-30">
                                                ...
                                                }} ref="closemodalSave">Cerrar</button>
                                            {
                                                this.state.fields["id"] === 0 ?
                                                    <ValidarPermiso
                                                        IdModuloPermisoValidar={ModuloPermiso.ControlPolitico.AgregarRespuestas}
                                                        DefaultTemplate={
                                                            <button className="btn btn-success pull-right"><i className="fa fa-check"></i> Guardar </button>
                                                        }
                                                    /> :
                                                    <ValidarPermiso
                                                        IdModuloPermisoValidar={ModuloPermiso.ControlPolitico.ModificarRespuestas}
                                                        DefaultTemplate={
                                                            <button className="btn btn-success pull-right"><i className="fa fa-check"></i> Guardar </button>
                                                        }
                                                    />
                    ...
                    <form action="">
                        <div className="mb-container">
                            <div className="mb-middle">
                                <div className="mb-title"><span className={`fa fa-${this.state.fields.activo ? "eraser" : "check"}`}></span> {this.state.fields.activo ? "Desactivar" : "Activar"} respuesta</div>
                                <div className="mb-content">
                                    <p>{`¿Está seguro que desea ${this.state.fields.activo ? "desactivar" : "activar"} la respuesta ${this.state.fields.nombre}? Puede volver a ${this.state.fields.activo ? "activarlo" : "desactivarlo"} en cualquier otro momento.`}</p>
                                </div>
                        ...
                        <div className="rowListado">
                            <div className="panel panel-default ">
                                <div className="panel-heading">
                                    <h3 className="panel-title"><span className="fa fa-list"></span> Listado de respuestas</h3>
...
export default ControlPoliticoRespuestas;