...
import GeografiaDataService from "../../../Services/Catalogo/Geografia.Service";
...
class Geografia extends Component {
    constructor(props) {
        ...
        this.state = {
            tableInfo: {
                columns: [
                    ...
                    },
                    {
                        Header: "Acciones",
                        columns: [
                            auth.tieneModuloPermiso(ModuloPermiso.Geografia.Obtener) ?
                            {
                                Header: "Editar",
                                id: "editar",
                                accessor: (str) => "editar",
                                Cell: (tableProps) => (
                                    <ValidarPermiso IdModuloPermisoValidar={ModuloPermiso.Geografia.Obtener} DefaultTemplate={
                                        <button
                                        data-toggle="modal"
                            ...
                            auth.tieneModuloPermiso(ModuloPermiso.Geografia.Obtener) ?
                            {
                                ...
                                Cell: (tableProps) => (
                                    <ValidarPermiso IdModuloPermisoValidar={ModuloPermiso.Geografia.Eliminar} DefaultTemplate={
                                        <button
                                        data-toggle="modal"
            ...
            desactivarGeografia: { id: 0, titulo: "", enabled: true }
        };
    }
    ...
    handlerDesactivar = (geografia) => {
        let desObj = { id: geografia.id, titulo: geografia.nombre, enabled: geografia.activo }
        this.setState({
            desactivarGeografia: desObj
        })
    }
    ...
    saveSubmit = async (e) => {
        ...
        if (this.state.fields.id === 0) {
            await GeografiaDataService.create(data)
                .then((response) => {
            ...
            await GeografiaDataService.update(
    ...
    deleteSubmit = async (e) => {
        ...
        let responseData;
        await GeografiaDataService.delete(this.state.desactivarGeografia.id)
            .then((response) => {
                responseData = response.data;
            })
            .catch(function (error) {
    ...
    getAll = async (idFilterActive, page, rows, search) => {
        ...
        let tableInfo = this.state.tableInfo;
        await GeografiaDataService.getAll(
            idFilterActive,
            search,
            page,
            rows
        ...
        await GeografiaDataService.getTotalRecords(idFilterActive, search)
            .then((response) => {
    ...
    getByID = async (id) => {
        this.setState({ loading: true });
        await GeografiaDataService.get(id)
            .then((response) => {
    ...
    render() {
        return (
            <div>
                            ...
                            <div className="panel panel-default ">
                                <div className="panel-heading">
                                    ...
                                    <ul className="panel-controls">
                                        ...
                                        <li>
                                            <ValidarPermiso IdModuloPermisoValidar={ModuloPermiso.Geografia.Nuevo} DefaultTemplate={
                                                <button
                                ...
                                <div className="panel-body">
                                    <div>
                                        <ValidarPermiso IdModuloPermisoValidar={ModuloPermiso.Geografia.ObtenerTodos} DefaultTemplate={
                                            <TableReactExtends
                                ...
                                >
                                            ...
                                            <div className="form-group">
                                                <div className="col-md-12">
                                                    ...
                                                    <Input
                                                        ...
                                                        inputplaceholder="Ingrese el nombre de la geografía"
                                                        inputValue={
                                                            this.state.fields[
                                                                "nombre"
                                                            ] || ""
                                    ...
                                    <div className="row">
                                        <div className="col-md-12 push-up-30">
                                            ...
                                            <ValidarPermiso IdModuloPermisoValidar={this.state.fields.id !== 0 ? ModuloPermiso.Geografia.Modificar : ModuloPermiso.Geografia.Nuevo} DefaultTemplate={
                ...
                <div className={`message-box message-box-${this.state.desactivarGeografia.enabled ? "danger" : "info"} animated fadeIn`} id="message-box-danger">
                        <form action="">
                            <div className="mb-container">
                                <div className="mb-middle">
                                    <div className="mb-title"><span className={`fa fa-${this.state.desactivarGeografia.enabled ? "eraser" : "check"}`}></span> {this.state.desactivarGeografia.enabled ? "Desactivar" : "Activar"} geografía</div>
                                    <div className="mb-content">
                                        <p>{`¿Está seguro que desea ${this.state.desactivarGeografia.enabled ? "desactivar" : "activar"} la geografía ${this.state.desactivarGeografia.titulo}? Puede volver a ${this.state.desactivarGeografia.enabled ? "activarla" : "desactivarla"} en cualquier otro momento.`}</p>
                                    </div>
                                    <div className="mb-footer">
                                    <ValidarPermiso IdModuloPermisoValidar={ModuloPermiso.Geografia.Eliminar} DefaultTemplate={
                                        <button type="button" className="btn btn-primary btn-lg pull-right" onClick={(e) => { this.deleteSubmit(e) }} >{this.state.desactivarGeografia.enabled ? "Desactivar" : "Activar"}</button>
                                    } />
