...
import CargoLegislativoDataService from "../../../Services/Catalogo/CargoLegislativo.Service";
...
class Cargos extends Component {
            ...
            tableInfo: {
                columns: [
                    ...
                    },
                    {
                        Header: "Acciones",
                        columns: [
                            auth.tieneModuloPermiso(ModuloPermiso.Cargo.Obtener) ?
                            {
                                Header: "Editar",
                                id: "editar",
                                accessor: (str) => "editar",
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar={ModuloPermiso.Cargo.Obtener}
                                        DefaultTemplate={
                                            <button
                                                data-toggle="modal"
                            ...
                            auth.tieneModuloPermiso(ModuloPermiso.Cargo.Eliminar) ?
                            {
                                ...
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar={ModuloPermiso.Cargo.Eliminar}
                                        DefaultTemplate={
                                            <button
                                                data-toggle="modal"
    ...
    saveSubmit = async (e) => {
        ...
        if (this.state.fields["id"] === 0) {
            await CargoLegislativoDataService.create(data).then((response) => {
            ...
            await CargoLegislativoDataService.update(this.state.fields["id"],data).then((response) => {
                    responseData = response.data;
                })
                .catch(function (error) {
                    errors = validForm.displayErrors(error.response.data,errors);
    ...
    deleteSubmit = async (e) => {
        ...
        let responseData;
        await CargoLegislativoDataService.delete(this.state.fields["id"]).then((response) => {
                responseData = response.data;
            })
            .catch(function (error) {
    ...
    getAll = async (idFilterActive, page, rows, search) => {
        ...
        let tableInfo = this.state.tableInfo;
        await CargoLegislativoDataService.getAll(
            idFilterActive,
            search,
            page,
            rows
        ...
        await CargoLegislativoDataService.getTotalRecords(idFilterActive,search).then((response) => {
    ...
    getByID = async (id) => {
        this.setState({ loading: true });
        await CargoLegislativoDataService.get(id)
            .then((response) => {
                            ...
                            <div className="panel panel-default ">
                                <div className="panel-heading">
                                    <h3 className="panel-title">
                                        ...
                                        Listado de cargos legislativos
                                    </h3>
                                    <ul className="panel-controls">
                                        ...
                                        <li>
                                        <ValidarPermiso
                                            IdModuloPermisoValidar={ModuloPermiso.Cargo.Nuevo}
                                            DefaultTemplate={
                                                <button
                                ...
                                <div className="panel-body">
                                    <div>
                                    <ValidarPermiso
                                        IdModuloPermisoValidar={ModuloPermiso.Cargo.ObtenerTodos}
                                        DefaultTemplate={
                                            <TableReactExtends
                    ...
                    <div className="modal-dialog modal-lg">
                        <div className="modal-content">
                            <div className="modal-header">
                                <button type="button" className="close" data-dismiss="modal" onClick={async () => {this.resetFields();}}>
                            ...
                            <div className="modal-body">
                                <form name="formAccion" className="form-horizontal" onSubmit={async (e) => { await this.saveSubmit(e); }}>
                                    ...
                                    <div className="row">
                                        <div className="col-md-12 push-up-30">
                                            <button type="button" className="btn btn-default" data-dismiss="modal" ref="closemodalSave" onClick={async () => { this.resetFields();}}>Cerrar</button>
                                            <ValidarPermiso
                                                IdModuloPermisoValidar={this.state.fields.id === 0 ? ModuloPermiso.Cargo.Nuevo :
                                                ModuloPermiso.Cargo.Modificar}
                                                DefaultTemplate={
                                                    <button className="btn btn-success pull-right">Guardar</button>
                                                }
                                            />
                                        </div>
                                    </div>
                        ...
                        <div className="mb-container">
                            <div className="mb-middle">
                                <div className="mb-title">
                                    <span className={`fa fa-${this.state.fields.activo ? "eraser" : "check"}`}></span>{" "}
                                ...
                                <div className="mb-content">
                                    <p>{`¿Está seguro que desea ${this.state.fields.activo ? "desactivar" : "activar"} el cargo ${this.state.fields.nombre}? Puede volver a ${this.state.fields.activo ? "activarlo" : "desactivarlo"} en cualquier otro momento.`}</p>
                                </div>
                                <div className="mb-footer">
                                    <button type="button" className="btn btn-primary btn-lg pull-right" onClick={async (e) => { await this.deleteSubmit(e); }}>
                                        {this.state.fields.activo ? "Desactivar" : "Activar"}
                                    </button>
                                    &nbsp;
