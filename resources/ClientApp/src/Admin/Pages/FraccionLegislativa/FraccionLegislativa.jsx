...
import FraccionLegislativaDataService from "../../../Services/Catalogo/FraccionLegislativa.Service";
...
const fieldsDeleteConst = { id: 0, nombre: '', imagen: [], user:'', activo: 0};
const errorsConst = { id: '', nombre: '', imagen: '', activo: '' };

class FraccionLegislativa extends Component {
    constructor(props) {
        super(props);
                        ...
                        columns: [
                            {
                                ...
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar={ModuloPermiso.FraccionLegislativa.Obtener}
                                        DefaultTemplate=
                                            {
                                                <button
                                                    data-toggle="modal"
                                                    data-target="#add-fraccionlegislativa"
                            ...
                            },
                            {
                                ...
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar={ModuloPermiso.FraccionLegislativa.Eliminar}
                                        DefaultTemplate=
                                            {
                                                <button
                                                    data-toggle="modal"
    ...
    handlerDesactivar = (fraccionLegislativa) => {
        let desActObj = { id: fraccionLegislativa.id, nombre: fraccionLegislativa.nombre, activo: fraccionLegislativa.activo  }
        this.setState({
    ...
    getByID = async (id) => {
        ...
        await FraccionLegislativaDataService.get(id)
            .then(response => {
                let fields = this.state.fields;
                fields = response.data[0];
                Object.assign(fields, { user: auth.username() });
                fields.imagen = fields.fraccion_legislativa_imagen;                
                this.setState({
                    fields: fields,
                    loading: false
                });
    ...
    saveSubmit = async (e) => {
        ...
        if (this.state.fields["id"] === 0) {
            await FraccionLegislativaDataService.create(data)
                .then(response => {
            ...
            await FraccionLegislativaDataService.update(this.state.fields["id"], data)
                .then(response => {
                    responseData = response.data;
                })
                .catch(function (error) {
    ...
    deleteSubmit = async (e) => {
        ...
        let responseData;
        await FraccionLegislativaDataService.delete(this.state.fieldsDelete["id"])
            .then(response => {
                responseData = response.data;
            })
            .catch(function (error) {
    ...
    getAll = async (idFilterActive) => {
        ...
        await FraccionLegislativaDataService.getAll(idFilterActive)
            .then(response => {
    ...
    render() {
        return (
            <div>
                ...
                <div className="modal" id="add-fraccionlegislativa" role="dialog" aria-labelledby="largeModalHead" aria-hidden="true">
                    <div className="modal-dialog modal-lg">
                        <div className="modal-content">
                            <div className="modal-header">
                                ...
                                <h4 className="modal-title" id="largeModalHead"><i className="fa fa-user"></i> {this.state.fields["id"] === 0 ? "Nueva" : "Editar"} fracción legislativa</h4>
                            </div>
                            <div className="modal-body">
                                <form className="form-horizontal custom" onSubmit={this.saveSubmit.bind(this)}>
                                    <Input divClass="col-md-12" inputName="idFraccionLegislativa" inputType="hidden" inputValue={this.state.fields["id"]} />
                                    <div className="row">
                                            ...
                                            <div className="form-group">
                                                <div className="col-md-12">
                                                    ...
                                                    <ImageForMultipleResolution
                                                        ...
                                                        resolutions={Constantes.FraccionLegislativaResolutions}
                                                        prefix="fraccionLegislativa" controlName="images-1" />
                                        ...
                                        <div className="col-md-12 push-up-30">
                                            ...
                                            <ValidarPermiso
                                                IdModuloPermisoValidar={
                                                    this.state.fields["id"] === 0 ?
                                                        ModuloPermiso.FraccionLegislativa.Nuevo
                                                        : ModuloPermiso.FraccionLegislativa.Modificar
                                                }
                                                DefaultTemplate=
                                                    {
                    ...
                    <form action="">
                        <div className="mb-container">
                            <div className="mb-middle">
                                <div className="mb-title"><span className={`fa fa-${this.state.fieldsDelete.activo ? "eraser" : "check"}`}></span> {this.state.fieldsDelete.activo ? "Desactivar" : "Activar"} fracción legislativa</div>
                                <div className="mb-content">
                                    <p>{`¿Está seguro que desea ${this.state.fieldsDelete.activo ? "desactivar" : "activar"} la fracción legislativa ${this.state.fieldsDelete.nombre}? Puede volver a ${this.state.fieldsDelete.activo ? "activarlo" : "desactivarlo"} en cualquier otro momento.`}</p>
                                </div>
                ...
                <ul className="breadcrumb push-down-0">
                    ...
                    <li><a href="/">Fracción legislativa</a></li>
                </ul>
                        ...
                        <div className="rowListado">
                            <div className="panel panel-default ">
                                <div className="panel-heading">
                                    <h3 className="panel-title"><span className="fa fa-list"></span> Listado de fracción legislativa</h3>
                                    <ul className="panel-controls">
                                        ...
                                        <li>
                                        <button type="button" data-toggle="modal" data-target="#add-fraccionlegislativa" className="btn btn-primary btn-block"><i className="fa fa-plus"></i> Nuevo registro</button>
                                        </li>
                                    </ul>
                                </div>
                                <div className="panel-body">
                                    <div>
                                        <ValidarPermiso
                                            IdModuloPermisoValidar={ModuloPermiso.FraccionLegislativa.ObtenerTodos}
                                            DefaultTemplate=
                                                {
                                                    <TableReact
...
export default FraccionLegislativa;
