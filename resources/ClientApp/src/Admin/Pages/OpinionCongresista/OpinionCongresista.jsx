...
const SelectCongresista = { value: '', label: 'Filtrar por congresista' };

class OpinionCongresista extends Component {
    constructor(props) {
        super(props);
        this.state = {
            tableInfo: {
                        ...
                        columns: [
                            {
                                ...
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar={ModuloPermiso.OpinionCongresista.Obtener}
                                        DefaultTemplate={
                                            <a href={`#/opinion-congresista-editar/${tableProps.row.values.id}`} data-id={tableProps.row.values.id}
                            ...
                            },
                            {
                                ...
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar={ModuloPermiso.OpinionCongresista.Eliminar}
                                        DefaultTemplate={
                                            <button
                                            data-toggle="modal"
            ...
            dataSelectAnno: [],
            filterCongresista: SelectCongresista,
    ...
    async componentDidMount() {
        await this.getAll(
            ...
            this.state.filterCongresista.value,
    ...
    handleFilterCongresista = async (selectcongresista) => {
        this.setState({ filterCongresista: selectcongresista });
        await this.getAll(
            this.state.filterActive.value,
            this.state.filterAnno.value,
            selectcongresista.value,
            this.state.tableInfo["page"],
            this.state.tableInfo["rows"],
            this.state.tableInfo["search"]
        );
    ...
    deleteSubmit = async (e) => {
        ...
        let responseData;
        await OpinionCongresistaDataService.delete(this.state.data["id"])
            .then((response) => {
    ...
    getAll = async (idFilterActive, annoPublicacion, congresista, page, rows, search) => {
        this.setState({ loading: true });
        let tableInfo = this.state.tableInfo;
        await OpinionCongresistaDataService.getAll(idFilterActive, annoPublicacion, congresista, page, rows, search)
            .then(response => {
    ...
    getComboAnno= async () => {
        await UtilsDataService.getComboOpinionCongresistaAnno()
            .then(response => {
                this.setState({ loading: true });
                let combo = [];
                response.data.forEach(i => {
                    combo.push({ value: i.fechaPublicacion, label: i.fechaPublicacion })
                })
    ...
    getComboCongresista = async () => {
        ...
        await UtilsDataService.getComboCargoCongresista().then(response => {
            let combo = [];
            let selected = { value: '', label: "Seleccione un congresista" };
            response.data.forEach(i => {
                combo.push({ value: i.id, label: i.nombre })
            })
            combo.unshift({ value: '', label: "Seleccione un congresista" })
            this.setState({
        ...
        return (
            <div>
                ...
                <ul className="breadcrumb push-down-0">
                    ...
                    <li><a href="/">Opiníon de Congresistas</a></li>
                </ul>
                        ...
                        <div className="rowListado">
                            <div className="panel panel-default ">
                                <div className="panel-heading">
                                    <h3 className="panel-title"><span className="fa fa-list"></span> Listado de opinión de Congresistas</h3>
                                    <ul className="panel-controls">
                                        <li>
                                            <div style={{ minWidth: "200px" }}>
                                                <Select
                                                    ...
                                                    selectValue={this.state.filterCongresista}
                                                    selectOnchange={this.handleFilterCongresista}
                                                    selectoptions={this.state.dataSelectCongresista}
                                                    selectIsSearchable={false}
                                                    selectclassNamePrefix="selectReact__value-container"
                                                    noOptionsMessage="Seleccione congresista"
                                                    spanClass=""
                                                    spanError=""
                                                />
                                            </div>
                                        ...
                                        <li>
                                            <ValidarPermiso
                                                IdModuloPermisoValidar = {ModuloPermiso.OpinionCongresista.Nuevo}
                                                DefaultTemplate = {
                                                    <a href={`#/opinion-congresista-crear`} className="btn btn-primary">
                                ...
                                <div className="panel-body">
                                    <div>
                                        <ValidarPermiso
                                            IdModuloPermisoValidar={ModuloPermiso.OpinionCongresista.ObtenerTodos}
                                            DefaultTemplate={
                                                <TableReact
...
export default OpinionCongresista;
