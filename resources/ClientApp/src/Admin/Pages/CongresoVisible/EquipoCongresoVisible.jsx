...
class EquipoCongresoVisible extends Component {
    constructor(props){
        super(props);
        const idcv =
            ...
            tableInfo: {
                columns: [
                    ...
                    },
                    {
                        Header: "Acciones",
                        columns: [    
                            auth.tieneModuloPermiso(ModuloPermiso.CongresoVisible.IndexIntegranteEquipo) ?
                            {
                                Header: 'Integrantes',
                                id: 'integrante',
                                accessor: (str) => 'integrante',
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar = {ModuloPermiso.CongresoVisible.IndexIntegranteEquipo}
                                        DefaultTemplate = {
                                            <a  href={`#/integrante-equipo/${tableProps.row.values.id}`} data-id={tableProps.row.values.id}
                                                className="btn btn-primary btn-block">
                                                            <i className="fas fa-handshake"></i> Integrantes
                                                </a>
                                        }
                                    />                                    
                                )
                            ...
                            auth.tieneModuloPermiso(ModuloPermiso.CongresoVisible.ObtenerEquipo) ?                   
                            {
                                Header: 'Editar',
                                id: 'editar',
                                accessor: (str) => 'editar',
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar = {ModuloPermiso.CongresoVisible.ObtenerEquipo}
                                        DefaultTemplate = {
                                            <a  href={`#/equipo-editar/${idcv}/${tableProps.row.values.id}`} data-id={tableProps.row.values.id}
                            ...
                            auth.tieneModuloPermiso(ModuloPermiso.CongresoVisible.EliminarEquipo) ?                   
                            {
                                ...
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar = {ModuloPermiso.CongresoVisible.EliminarEquipo}
                                        DefaultTemplate = {
                                            <button
                                            data-toggle="modal"
    ...
    deleteSubmit = async (e) => {
        ...
        let responseData;
        await congresoVisibleDataService.deleteEquipo(this.state.fields["id"])
            .then((response) => {
    ...
    getAll = async (idFilterActive,page,rows,search)=>{
        this.setState({loading:true});
        let tableInfo = this.state.tableInfo;
        await congresoVisibleDataService.getAllEquipo(idFilterActive,page,rows,search)
            .then(response =>{
                        ...
                        <div className="rowListado">
                            <div className="panel panel-default ">
                                <div className="panel-heading">
                                    <h3 className="panel-title"><span className="fas fa-user-friends"></span> Listado de equipos</h3>
                                    <ul className="panel-controls">
                                        ...
                                        <li>
                                            <a href={`#/equipo-crear/${this.state.idcv}`} className="btn btn-primary">
                                ...
                                <div className="panel-body">
                                    <div>      
                                        <ValidarPermiso
                                            IdModuloPermisoValidar = {ModuloPermiso.CongresoVisible.ObtenerTodosEquipo}
                                            DefaultTemplate = {
                                                <TableReact
                            ...
                            <div className="mb-container">
                                <div className="mb-middle">
                                    ...
                                    <div className="mb-content">
                                        <p>{`¿Está seguro que desea ${this.state.fields.activo ? "desactivar" : "activar"} el equipo ${this.state.fields.titulo}? Puede volver a ${this.state.fields.activo ? "activarla" : "desactivarla"} en cualquier otro momento.`}</p>
                                    </div>
...
export default EquipoCongresoVisible;
