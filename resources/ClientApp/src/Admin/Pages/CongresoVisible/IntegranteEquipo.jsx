...
class IntegranteEquipo extends Component {
    constructor(props){
        super(props);
        const idequipo =        
            ...
            tableInfo: {
                columns: [
                    ...
                    },
                    {
                        Header: "Acciones",
                        columns: [   
                            auth.tieneModuloPermiso(ModuloPermiso.CongresoVisible.ObtenerIntegranteEquipo) ?                   
                            {
                                Header: 'Editar',
                                id: 'editar',
                                accessor: (str) => 'editar',
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar = {ModuloPermiso.CongresoVisible.ObtenerIntegranteEquipo}
                                        DefaultTemplate = {
                                            <a  href={`#/integrante-editar/${idequipo}/${tableProps.row.values.id}`} data-id={tableProps.row.values.id}
                            ...
                            auth.tieneModuloPermiso(ModuloPermiso.CongresoVisible.EliminarIntegranteEquipo) ?                   
                            {
                                ...
                                Cell: (tableProps) => (
                                    <ValidarPermiso
                                        IdModuloPermisoValidar = {ModuloPermiso.CongresoVisible.EliminarIntegranteEquipo}
                                        DefaultTemplate = {
                                            <button
                                        data-toggle="modal"
    ...
    deleteSubmit = async (e) => {
        ...
        let responseData;
        await congresoVisibleDataService.delete(this.state.fields["id"])
            .then((response) => {
    ...
    getAll = async (idFilterActive,page,rows,search)=>{
        this.setState({loading:true});
        let tableInfo = this.state.tableInfo;
        await congresoVisibleDataService.getAllIntegrante(idFilterActive,page,rows,search)
            .then(response =>{
                        ...
                        <div className="rowListado">
                            <div className="panel panel-default ">
                                <div className="panel-heading">
                                    <h3 className="panel-title"><span className="fas fa-user-friends"></span> Listado de integrantes</h3>
                                    <ul className="panel-controls">
                                        ...
                                        <li>
                                            <a href={`#/integrante-crear/${this.state.idequipo}`} className="btn btn-primary">
                                ...
                                <div className="panel-body">
                                    <div> 
                                        <ValidarPermiso
                                            IdModuloPermisoValidar= {ModuloPermiso.CongresoVisible.ObtenerTodosIntegranteEquipo}
                                            DefaultTemplate = {
                                                <TableReact
                            ...
                            <div className="mb-container">
                                <div className="mb-middle">
                                    ...
                                    <div className="mb-content">
                                        <p>{`¿Está seguro que desea ${this.state.fields.activo ? "desactivar" : "activar"} el equipo ${this.state.fields.nombre}? Puede volver a ${this.state.fields.activo ? "activarla" : "desactivarla"} en cualquier otro momento.`}</p>
                                    </div>
