...
import './GoDownButton.css';

const GoDownButton = ({ className = "", gtdtarget = 1}) => {
    return (
        <div className={"GoDownButtonContainer " + className} onClick={()=>{GoToDownFunction(gtdtarget)}}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80.06 90.04">
            <defs>
                <style>
...
function GoToDownFunction(gtdtarget){
    var elmnt = document.querySelector(`[gtdtarget='${gtdtarget}']`);
    if(elmnt)
        elmnt.scrollIntoView();
    else
        console.warn("El compontente GoToDown necesita que se defina un element taget agregando gtdtarget='true' a la etiqueta.");
}
