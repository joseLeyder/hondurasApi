...
const InputPercent = ({ name = "", noShowValue = false, value = 50, color = "#333", isDinamyc = false, lineCapStroke = "round", fontSize = 3, cls = "", id = "", fontColor = "#333" }) => {
    if (value > 100) {
                ...
                <div className={cls + " InputPercentContainer"} id={id}>
                    <div className="CPercent">
                        <svg viewBox={`0 0 150 150`} version="1.1" xmlns="http://www.w3.org/2000/svg">
                            ...
                            {
                                noShowValue ? "" :
                                <text style={{ fontSize: fontSize + "em"}} x={tx} y={ty} fill={fontColor}>{Math.floor(value)}</text>
                            }
                        </svg>
                    </div>
                </div>
            ...
            return (
                <>
                    <div className={cls + " InputPercentContainer dinamyc"} id={id}>
                        ...
                        <input style={{ fontSize: fontSize + "em" }} type="text" name={name} value={Math.floor(value)} onKeyDown={(e) => { onChangeNumber(e) }} />
                    </div>
                </>
            )
        }
